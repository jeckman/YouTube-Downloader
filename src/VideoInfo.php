...
class VideoInfo
{
	...
	/**
	  * Check if VideoInfo have chiper signature from StreamMap
	 *
	 * @param StreamMap $stream_map
	 * @return bool
	 */
	public function checkcipher()
	{
		$chek = explode(',', $this->data['url_encoded_fmt_stream_map']);
		foreach ($chek as $format){
			$url = urldecode($format);
